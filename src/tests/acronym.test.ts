...
describe('Testing Acronym', () => {
    ...
    describe('[POST] /acronym', () => {
        it('reponse status 201, created', async () => {
            ...
            await request(app.getServer())
            .delete(`${acronymRoute.path}/${newData.acronym}`)
            .expect(200)
        })
    })
    ...
    describe('[PUT] /acronym/:acronym', () => {
        it('response status 200, update', async () => {
            ...
            await request(app.getServer())
                .put(`${acronymRoute.path}/${newData.acronym}`)
                .send(newData)
                .expect(200, { data: convertToObject([newData]), message: 'updated' })
